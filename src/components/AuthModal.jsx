  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        alert("Check your email for confirmation (if using email confirmations).");
        // Optionally auto sign-in after signUp depending on Supabase settings.
  ...
  return (
    <div style={styles.backdrop}>
      <div style={styles.modal} role="dialog" aria-modal="true" aria-label="Sign in">
        <h3 style={{ marginTop: 0 }}>{mode === "signup" ? "Create an account" : "Sign in"}</h3>
...
const styles = {
  backdrop: {
    ...
    background: "rgba(10,11,12,0.35)", zIndex: 60,
  },
  modal: {
    width: 420, maxWidth: "94%", borderRadius: 12, background: "#fff", padding: 18, boxShadow: "0 12px 40px rgba(0,0,0,0.15)"
  },
