...
/**
 * Root-level getChallenges for Vercel
 * Defensive: clear 500 when server env missing; accepts token via header/query/cookie.
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ ok: false, error: "server-missing-config", message: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing/invalid on server." });
  }
  ...
  try {
    ...
    if (!authToken) return res.status(200).json({ ok: true, challenges: publicChallenges, custom: [], user: null });
    ...
    if (!user) return res.status(200).json({ ok: true, challenges: publicChallenges, custom: [], user: null });
    ...
    const userMeta = { id: user.id, email: user.email, credits: creditsData?.credits ?? 0, xp: totalXp };
